# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 02:16:02 2023

@author: adamWolf

Open the prepared clean spanish text file,
...
data = basic_functions.open_text('HP-spanish_clean.txt').read()
...
plt.title('Longest word in HP-spanish: ' + ' '.join(x for x in words if len(x)==max(lengths)))

plt.show()

clear_file = ' '.join(words)
basic_functions.write_text('HP-spanish_final.txt', clear_file)